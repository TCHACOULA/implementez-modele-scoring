...
app.layout = html.Div(
    children=[
        ...
        dash_table.DataTable(
            data=df_application_test.head(10).to_dict("records"), page_size=10
        ),
        html.Hr(),
        html.Div(
            [
                ...
                dbc.Row([dbc.Col(html.Div(id="prediction", children=[]))]),
            ]
        ),
        html.Hr(),
        html.Div(
        ...
        dcc.Dropdown(
            options=df_application_test.columns[1:],
            value="CODE_GENDER",
            id="feature-dropdown",
        ),
        ...
        html.Div(
            [
                dcc.Graph(id="global-importance", figure={}),
            ]
        ),
        html.Hr(),
...
def update_prediction(client_id):
    ...
    try:
        ...
        logging.error(f"Une erreur est souvenue lors de la prédiction: {e}")
...
def update_graph(col_chosen, client_id):
    ...
    fig = px.histogram(
        ...
        x=col_chosen,
        color="TARGET",
        title=f"Distribution en fonction de {col_chosen}",
    )
    fig.add_vline(
...
def update_graph_bi(feature_bi1, feature_bi2, client_id):
    ...
    fig = px.scatter(
        ...
        y=feature_bi2,
        color="TARGET",
        title=f"Analyse bivariée en fonction de {feature_bi1} et {feature_bi2}",
    )
    fig.add_trace(
        px.scatter(
...
def create_global_importance_graph(feature_names):
    ...
    features = [
        feature_names[int(col.split("_")[1])] for col in df_feature_importance.feature
    ]
    fig = px.bar(
        y=features,
...
def update_local_feature_importance(client_id):
    ...
    # Créer les graphiques d'importance globale et locale
    local_fig = create_local_importance_graph(
        int(client_id)
    )  # Afficher les valeurs pour le premier échantillon
